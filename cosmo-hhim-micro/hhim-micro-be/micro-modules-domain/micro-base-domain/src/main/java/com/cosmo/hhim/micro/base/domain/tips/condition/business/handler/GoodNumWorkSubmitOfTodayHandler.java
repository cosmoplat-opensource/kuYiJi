 ...
 * @desc 检验当前员工今天的记工总数超过{?}%
 * @createTime 2023/2/10
 */
@Slf4j
@Component
public class GoodNumWorkSubmitOfTodayHandler extends AbstractBusinessHandler {
    ...
    protected BusinessHandlerResult executeHandler(BusinessHandlerParam param) {
        ...
        // 2.检验今天的记工总数是否超过80%的员工
        // 按照用户分组统计今天已审核的记工总数
        MicroWorkSubmitDto queryParam = new MicroWorkSubmitDto();
        queryParam.setStartDate(DateUtil.beginOfDay(nowDate).toJdkDate());
        queryParam.setEndDate(DateUtil.endOfDay(nowDate).toJdkDate());
        ...
        if (!CollectionUtils.isEmpty(groupSumResult)) {
            ...
            // 判断当前用户记工总数是否达到业务条件的阈值（注意：这里排除掉自己，只有自己记工时，不进行提示）
            if (null != index && collect.size() > 1
                ...
                String showRatio = String.valueOf(startRatio.multiply(new BigDecimal(100)).setScale(0, RoundingMode.HALF_UP));
                placeHolderList.add(showRatio);
