<template>
  <scroll-view
    scroll-y
    class="m-h-360 w-100 box bg-fff overflow-auto absolute product-item-wrapper"
    style="z-index: 99"
  >
    <template v-if="getDataList.length > 0">
      ...
      >
        ...
        <view class="flex-1 flex align-center">
          <rich-text :nodes="item.nodeName" v-if="item.userName" />
          <view v-else class="color-5a6f82 ml-16">{{ item.nickName }}</view>
          <rich-text v-if="item.userName" :nodes="item.nodeCode" />
        </view>
      </view>
    </template>
    <template v-else>
      <view class="product-item box flex align-center">
        <image src="/static/images/icon_s_b8.svg" class="icon-24" />
        <text class="font-28 color-b8b8b8 ml-16 flex-1">未查到相关员工，请重新输入</text>
...
const getDataList = computed(() => {
  return props.staffList
    .filter((v) => (!props.showInput && v.userName) || props.showInput)
    .map((item: any) => {
      const tempName = escapeHtml(formatStr(item.nickName, 10)).replace(
      ...
      const tempCode = escapeHtml(formatStr(item.userName, 10)).replace(
        new RegExp(props.searchValue, 'g'),
        `<span style="color: #0066ff;">${escapeHtml(props.searchValue)}</span>`
      )
