<template>
  <uni-popup ref="popupStaff" type="bottom" @maskClick="handleCancel" :safe-area="false">
    <view class="bg-fff flex flex-col justify-start font-28 rounded-16-top">
      ...
      <view class="bg-f7f8e7 py-24 px-32 box flex align-center">
        <view class="font-28 color-333 bold">{{ productItem.itemName }}</view>
        <view class="font-28 color-999 bold">({{ productItem.itemCode }})</view>
      </view>
      <view class="min-h-500">
        ...
        <view class="flex flex-col mt-16 h-500 overflow-auto" v-if="dataList.length">
          ...
          >
            <view class="bold flex align-center flex-1 single" :class="item.checked ? 'color-0066ff' : 'color-333'">
              {{ item.itemName }}
            </view>
        ...
        <view v-else class="py-128 flex flex-col align-center">
          <template v-if="productItem.standard">
            <h-empty tipsWord="未查到相关工序" />
          </template>
          <template v-else>
...
import index from '@/pages-rejects/index.vue'
...
function open(product) {
  ...
  tempProcessList.value = product.processList || []
  searchInput.value = ''
  initData()
