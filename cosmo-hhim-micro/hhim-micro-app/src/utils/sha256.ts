...
/** 字符串转 UTF-8 字节（兼容小程序，不依赖 TextEncoder） */
...
/**
 * 计算字符串的 SHA-256 摘要（hex 小写），与 Java MessageDigest("SHA-256") 输出一致。
 */
export function sha256(input: string): string {
  const bytes = utf8Encode(input)
  const bitLen = bytes.length * 8

  // 消息填充：0x80 + 0 填充至 56 mod 64，末尾 8 字节大端长度
...
/**
 * 埋点事件编码:SHA-256 取前 32 位(128 位熵,碰撞概率可忽略)。
 * 与后端 MicroUserFacadeServiceImpl 的截断口径一致,
 * 兼容 micro_user_event_tracking.event_code varchar(40) 列宽。
 */
