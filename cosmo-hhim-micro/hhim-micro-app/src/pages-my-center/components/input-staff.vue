    ...
    <view class="bg-fff box flex flex-col justify-start font-28 rounded-16-top">
      ...
      <view class="min-h-500">
        ...
        <view class="flex flex-col mt-24 h-500 overflow-auto" v-if="staffList.length">
          ...
          >
            <view class="bg-f3f3f5 rounded-8 h-32 flex-ac px-4 box font-24 color-5a6f82 mr-16">{{
              getRoleList(item.roleCode)
            }}</view>
            <template v-if="itemSelectList.some((itemSelect) => itemSelect.id === item.id)">
...
function handleSelectProduct(val) {
  const index = itemSelectList.value.findIndex((itemSelect) => itemSelect.id === val.id)

  if (index < 0 && itemSelectList.value.length < 3) {
    itemSelectList.value.push(val)
  } else if (itemSelectList.value.length === 3) {
    uni.showToast({ title: '最多选择3个员工', icon: 'none' })
    return
  } else {
    itemSelectList.value.splice(index, 1)
  }
}

function initData() {
  _get({ url: '/user/all/list', data: { key: searchInput.value } }).then((res) => {
    staffList.value = res.rows
  })
}

function confirmPopup() {
  if (!itemSelectList.value.length) {
    uni.showToast({ title: '请选择员工', icon: 'none' })
    return
  }
  emits('staffSelected', [...itemSelectList.value])
  handleCancel()
}
