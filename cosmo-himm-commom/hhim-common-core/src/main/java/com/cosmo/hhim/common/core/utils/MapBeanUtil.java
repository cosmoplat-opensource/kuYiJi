...
public class MapBeanUtil {
    ...
    /**
     * 将一个Map转换成一个JavaBean(Map中的Key和传出来的JavaBean的属性名完全一致)
    ...
    public static <T> T mapToBean(Class<T> clazz, Map map) {
        ...
        try {
            ...
            log.error("mapToBean失败: {}", clazz.getName(), e);
        }
        return result;
    }

    /**
     * 将一个Map转换成一个JavaBean(Map中的Key包含下划线, 下划线转成驼峰)
        ...
        try {
            ...
            for (PropertyDescriptor descriptor : propertyDescriptors) {
                ...
                if (map.containsKey(propertyName) || map.containsKey(camelToUnderlinePropertyName)) {
                    Object value = map.get(propertyName);
                    if (value == null) {
                        value = map.get(camelToUnderlinePropertyName);
                    }
                    if ("".equals(value)) {
                        value = null;
                    }
            ...
            log.error("mapCamelToBean失败: {}", clazz.getName(), e);
        }
        return result;
    }

    /**
     * 将一个 JavaBean 对象转化为一个 Map(属性与Map的key完全一致)
    ...
    public static Map beanToMap(Object bean) {
        ...
        try {
            ...
            log.error("beanToMap失败: {}", bean.getClass().getName(), e);
        }
        return resultMap;
    }
    ...
    /**
     * 将一个JavaBean对象转化为一个Map(驼峰属性被转成带下划线的key)
    ...
    public static Map beanToMapUnderLine(Object bean) {
        ...
        try {
            ...
            log.error("beanToMapUnderLine失败: {}", bean.getClass().getName(), e);
        }
        return resultMap;
    }
