...
import com.cosmo.hhim.common.core.constant.ProxyConstant;
...
public abstract class AbstractAutoScanProxy extends AbstractAutoProxyCreator {
    ...
    /**
     * 作用：接收容器里产生的Bean的类及名称，以及Bean的原始对象。
     * 可以对Spring创建的Bean做代码判断，通过返回值告诉Spring是否要对这个Bean做动态代理
     * @param beanClass
     * @param beanName
     * @param targetSource
     * @return DO_NOT_PROXY 对bean不进行代理 / PROXY_WITHOUT_ADDITIONAL_INTERCEPTORS 不创建额外的代理，使用spring默认的代理
     */
    @Override
