...
public class ExportCommonService {
     ...
     * @param targetId 目标ID
     */
    public void getAllExcelFieldCustom(String[] exclusions, String targetId, Field[] fields,
                                       List<ExcelExportEntity> excelParams, Class<?> pojoClass,
                                       List<Method> getMethods, ExcelEntity excelGroup, Map<String, String> custom) throws Exception {
        for (Map.Entry map : custom.entrySet()) {
            ExcelExportEntity excelEntity = new ExcelExportEntity((String) map.getValue(), map.getKey());
            excelParams.add(excelEntity);
        }
    ...
    public Object getCellValue(ExcelExportEntity entity, Object obj) throws Exception {
        ...
        if (obj instanceof Map) {
            ...
            if (entity.getMethods() == null && entity.getMethod() == null) {
                Method m = obj.getClass().getMethod("getExtendContent");
                JSONObject jsonObject = JSONObject.parseObject((String) m.invoke(obj));
                if (jsonObject == null) {
                    value = PoiPublicUtil.getParamsValue(entity.getKey().toString(), obj);
                } else {
                    value = jsonObject.get(entity.getKey());
                }
            } else {
