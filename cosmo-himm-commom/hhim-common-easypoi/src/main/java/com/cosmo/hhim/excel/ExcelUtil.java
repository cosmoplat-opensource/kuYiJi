...
import com.cosmo.hhim.utils.ExcelUtilExt;
...
//        System.out.println("datas:" + (datas == null ? null : datas.stream().map(DictData::getDictLabel).collect(Collectors.joining())));
        ...
        try {
            ...
            if (wb != null) {
                try {
                    ...
                    log.warn("close workbook failed: {}", e1.getMessage(), e1);
                }
            }
        ...
        try {
            ...
            if (outputStream != null) {
                try {
                    ...
                    log.warn("close output stream failed: {}", e1.getMessage(), e1);
                }
            }
        }
    }
    ...
    private void addStatisticsData(Integer index, String text, Excel entity) {
        if (entity != null && entity.isStatistics()) {
            ...
            try {
                ...
                log.warn("parse statistics number failed: {}", e.getMessage(), e);
            }
    ...
    public void addStatisticsRow() {
        if (statistics.size() > 0) {
            ...
            for (Integer key : keys) {
                ...
                cell.setCellStyle(styles.get("total"));
                cell.setCellValue(DOUBLE_FORMAT.get().format(statistics.get(key)));
            }
            statistics.clear();
        }
    }
    ...
    private Object getTargetValue(T vo, Field field, Excel excel) throws Exception {
        // 通过公共 getter 读取字段值（避免 setAccessible 修改访问权限修饰符）
        Object o = getValue(vo, field.getName());
    ...
    private Object getValue(Object o, String name) throws Exception {
        if (StringUtils.isNotNull(o) && StringUtils.isNotEmpty(name)) {
            o = getFieldValueByGetter(o, name);
        }
        return o;
    }
     ...
     * getMethod 会自动沿继承链查找公共 getter，较 getDeclaredField 更健壮
     */
    private Object getFieldValueByGetter(Object o, String fieldName) {
        if (o == null || fieldName == null) {
            return null;
        }
        String suffix = Character.toUpperCase(fieldName.charAt(0)) + fieldName.substring(1);
        try {
            return o.getClass().getMethod("get" + suffix).invoke(o);
        } catch (Exception e) {
            try {
                return o.getClass().getMethod("is" + suffix).invoke(o);
            } catch (Exception e2) {
                return null;
            }
        }
