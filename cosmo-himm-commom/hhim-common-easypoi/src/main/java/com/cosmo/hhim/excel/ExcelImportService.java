...
//                    saveImage(entity, picId, param.getExcelParams(), titleString, pictures, params);
        ...
        if (params.isConcurrentTask()) {
            ...
            while (rows.hasNext()
                ...
                if (row.getLastCellNum() < 0) {
                    continue;
                }else if(isAllRowEmpty(row, sheet.getRow(sheet.getFirstRowNum()))){
                    continue;
                }
    ...
    public ExcelImportResult importExcelByIs(InputStream inputstream, Class<?> pojoClass,
                                             ImportParams params, boolean needMore, int importRowNum) throws Exception {
        if (importRowNum == 0) {
            importRowNum = 50000;
        }
...
//            while (it.hasNext()) {
//                int a = it.next().getLastRowNum();
//                if (a > importRowNum) {
//                    throw new RuntimeException("导入行数超过" + importRowNum + "，请分批导入");
//                }
//            }
...
//                }
                ...
                result.addAll(importExcel(result, book.getSheetAt(i), pojoClass, params, null));
                if (LOGGER.isDebugEnabled()) {
...
//                    }
    ...
    private void saveImage(Object object, String picId, Map<String, ExcelImportEntity> excelParams,
        ...
        String fileName = "pic" + Math.round(SECURE_RANDOM.nextDouble() * 100000000000L);
